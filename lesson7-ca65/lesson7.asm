...
.ZEROPAGE ; premium real estate for global variables
ptr:           .word ; Base pointer used for indirection
r0:            .byte ; Simple re-usable byte register for subroutines

ret0:          .byte ; Used for temp space for swapping return values after RTS
ret1:          .byte ; RetN are protected by interrupt request handlers
ret2:          .byte ; These values will be overwritten on any JSR
ret3:          .byte ; So swap them back and store them on the stack if needed

state:         .byte ; Holds the game state
frame_counter: .byte ; Alloc 1 byte for the frame counter
current_frame: .byte ; Alloc 1 byte for the current frame
controller:    .byte ; Holds bitmask of controller state

p1_score:      .word ; Kept as decimal digits
p2_score:      .word ; Kept as decimal digits
time:          .res 4 ; Kept as 4 decimal digits

.DATA ; Rest of RAM. First 255 bytes are stack. Next 255 are sprite DMA. Rest are free to use
.org $0100      ;; SPRITE_AREA is offset 256 bytes from the top of data
...
.segment "iNes" ; Defines a header segment for emulators. When using a real cart, we would strip these bytes and just use the zeropage as the start
  .word $4e, $45, $53, $1a ; ines filetype magic bytes
  .word $01 ; # of 16kb PRG segments
  .word $01 ; # of 8kb CHR segments
  .word $00 ; # Mapper=0, NROM aka no bank swapping
  .word $01 ; # Nametable mirroring in the PPU

.segment "PRG1" ; PRG1 bank starts at $c000
  .include "lib.asm"
...
.segment "PRG2" 1 ; inesprg bank 2 - 8kb, starts at $e000
...
strings:
  ...
  .dword NMI   ; Non-maskable interrupt (ie VBLANK)
  .dword RESET ; Processor turns on or reset button is pressed
  .dword 0     ; Other interrupts. Ignore for now
